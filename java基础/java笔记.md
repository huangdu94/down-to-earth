...
10. `final`关键字
	+ 使用final修饰过的都是不可改变的
	+ final修饰变量：不可被重新赋值，变量名建议全部大写
	+ final修饰方法：不可被重写，重载不会受到限制
	+ final修饰类：不可被继承，该类不能作为任何类的父类
...
2. 继承
	...
	+ 子类继承父类之后，子类可以调用父类的属性和方法(传递性)，也可以重写父类的属性和方法，还可以增加自己的属性和方法
	+ 重写(遵循"两同两小一大"原则)
	...
	+ 在调用子类的构造器的时候，如果没有显示的写出super，那么编译器会在子类构造方法开头默认加上`super();`
3. 多态
