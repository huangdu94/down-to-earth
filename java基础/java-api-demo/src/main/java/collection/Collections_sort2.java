...
public class Collections_sort2 {
	public static void main(String[] args) {
		...
		/*
		 * Collections的sort方法在排序List集合时要求该集合的元素必须实现：Comparable接口.该接口规定了元素可以比较大小.
		 */
//		Collections.sort(list);
		/*
		 * 排序自定义类型元素时，推荐使用下面的重载sort方法.该方法要求传入一个额外的比较器,使用匿名内部类即可.这样一句代码就可以完成排序操作.而无需在PointTwo类中实现接口并为其提供一个方法.
		 * 当我们时用某个功能时，该功能要求我们为其修改代码越多，侵入性越强.实际开发中应该尽量避免侵入性强的功能.
		 */
