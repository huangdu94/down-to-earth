 ...
 * 并且也是根据key元素得到位置来检索数组中的该数组键值对以此省去了遍历数组的过程.
 * <p>
 * 但是使用HashMap时应当尽量避免出现链表的情况.HashMap中链表产生原因：
 * HashMap是根据key元素的hashcode方法返回的数字经过散列计算得到改组键值对应当存放在数组的哪个位置，
 * 而根据key元素equals比较查看当前key是否已经存在于Map中，若存在则直接替换该位置这个key所对应的value，
 * 但是若数组该位置已经存在键值对，并且key与当前准备存放的键值对中的key的equals比较不同时，
 * 则会在数组当前位置产生链表.这样在将来根据key检索时会出现遍历链表的情况，这样就会降低HashMap查询性能.
 * <p>
 ...
 * 2：一致性，即：当两个对象equals比较为true时，hashcode返回的数字应当相等.
 * 反之亦然,因为若两个对象equals比较不同但是hashcode相等时在HashMap中会产生链表.
